...
*AI-assisted code comprehension*: downstream tasks, like code search, code completion, or automated code documentation, require a tool for creating deep semantic meaning of code tokens, functions, and entire programs

*Bidirectional encoder representations from transformer (BERT)*: general-purpose encoder architecture based on multi-head self-attention to capture rich contextual representations

*Pre-trained via Masked Language Modeling*: training on large text corpora with random tokens masked, so that the model learns to predict the missing tokens using context
...
*Goal:* An Erlang-specific BERT model that we call *ErlangBERT*
...
Contributions:
- Specialized BERT model for Erlang language
- Reproducible corpus creation methodology
- Pre-trained BERT model specialized for Erlang (full retraining of LoRA)
