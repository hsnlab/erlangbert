...
#+DATE: 2025-07-29
#+OPTIONS: H:2 toc:nil num:nil timestamp:nil
#+LATEX_CLASS: beamer
...
* Introduction
** Introduction
Why Erlang-Specific Code Embeddings?
- Erlang has unique language features not well-captured by general models
- Pattern matching and message passing require specialized understanding
- OTP behaviors and actor model semantics need domain-specific representation
- Limited representation in existing multilingual code models

** Problem Statement
Current Limitations
- General code models (CodeBERT, GraphCodeBERT) trained primarily on mainstream languages
- Erlang represents < 1% of typical training corpora
- Functional programming paradigms underrepresented
- Actor model concurrency patterns not well understood

** GraphCodeBERT
GraphCodeBERT
- Extension of CodeBERT with data flow graph integration
- Pre-trained on code-text pairs and code structure
- Combines natural language and programming language understanding
- Achieves state-of-the-art results on code-related tasks
...
- First specialized BERT model for Erlang language
- Reproducible corpus creation methodology
...
- **Resources**: Reproducible corpus creation and evaluation framework
- **Community**: Open-source tools for Erlang code analysis and tooling.
  Code available on GitHub: [[https://github.com/hsnlab/erlangbert][hsnlab/erlangbert]]

** Future Work
- Implement LoRA adaptation
- Implement and evaluate on downstream tasks from the paper
  (e.g., code search)
